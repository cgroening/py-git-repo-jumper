...
class TestOpenGitTool:
    def test_raises_configured_git_tool_not_found_on_file_not_found(
        self, tmp_path
    ):
        client = SubprocessGitClient()
    ...
    def test_raises_configured_git_tool_not_found_on_called_process_error(
        self, tmp_path
    ):
        client = SubprocessGitClient()
        with patch(
...
class TestGetGithubRepoName:
    def test_returns_dash_when_no_git_dir(self, tmp_path):
        result = SubprocessGitClient._get_github_repo_name(
            str(tmp_path), 'user'
        )
        assert result == '-'
...
class TestGenerateStatusSummary:
    def test_clean_repo_returns_checkmark(self, tmp_path):
        ...
        with patch('subprocess.run', side_effect=mock_run):
            status, changes = SubprocessGitClient._generate_status_summary(
                tmp_path
            )
