    ...
    def _adjust_columns(self, strategy: ColumnAdjustmentStrategy, columns_sorted_by_prio: list[ColumnConfig]):
        ...
        for i, column_config in enumerate(columns_sorted_by_prio):
            ...
            for next_column_config in columns_sorted_by_prio[i+1:]:
                if do_shrink:
                    has_same_prio = next_column_config.shrink_priority == column_config.shrink_priority
                else:
                    has_same_prio = next_column_config.stretch_priority == column_config.stretch_priority
            ...
            if column_config.name in self._calculated_widths:
                continue
            ...
            # Distribute budget equally among columns in this priority group,
            # respecting max_width (stretch) or floor of 0 (shrink).
            # Excess from capped columns gets redistributed within the group.
            ...
            while remaining_budget > 0 and remaining_cols:
                ...
                for j, col in enumerate(remaining_cols):
                    ...
                    if do_shrink:
                        new_width = self._calculated_widths[col.name] - adjustment
                        if new_width < 0:
                            distributed += self._calculated_widths[col.name]
                            self._calculated_widths[col.name] = 0
                        else:
                            self._calculated_widths[col.name] = new_width
                            distributed += adjustment
                            next_remaining.append(col)
                    else:
                        new_width = self._calculated_widths[col.name] + adjustment
                        if new_width > col.max_width:
                            distributed += col.max_width - self._calculated_widths[col.name]
                            self._calculated_widths[col.name] = col.max_width
                        else:
                            self._calculated_widths[col.name] = new_width
                            distributed += adjustment
                            next_remaining.append(col)
            ...
            self._width_budget = -remaining_budget if do_shrink else remaining_budget
