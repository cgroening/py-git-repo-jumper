...
def format_repo_table_line(repo: Dict[str, str], git_info: Dict[str, any],
    ...
    Returns a formatted string with fixed column widths.
    """
...
def create_table_header(max_name: int = 20, max_branch: int = 10,
                       max_status: int = 15, max_github: int = 20,
                       max_path: int = 30) -> str:
    """Creates a table header for the fuzzy finder."""
    name_header = "Name".ljust(max_name)
    branch_header = "Branch".ljust(max_branch)
    status_header = "Status".ljust(max_status)
    github_header = "GitHub".ljust(max_github)
    path_header = "Path".ljust(max_path)

    header = f"  {name_header} │ {branch_header} │ {status_header} │ {github_header} │ {path_header}"
...
def cleanup_old_last_repo(config_dir: Path, keep_last_repo: bool = True):
    """
    Optionally cleans up old last_repo.txt file.

    Args:
        config_dir: Directory where config file is located
        keep_last_repo: If False, deletes existing last_repo.txt
    """
    last_repo_file = config_dir / "last_repo.txt"
...
def main():
    ...
    parser.add_argument(
        "--clean",
        action="store_true",
        help="Remove last_repo.txt before starting (clean slate)",
    )
    ...
    # Determine directory for last_repo.txt (same as config file)
    config_dir = config_file_path.parent
    last_repo_file = config_dir / "last_repo.txt"

    # Optional cleanup of old last_repo.txt
    if args.clean:
    ...
    console.print(create_table_header(max_name, max_branch, 15, max_github, 35))
    console.print()
    ...
    if selected:
        ...
        # Save path in last_repo.txt in same directory as config
        try:
